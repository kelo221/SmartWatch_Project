...
type User struct {
	...
	// Elevated holds the value of the "elevated" field.
	...
	for i := range columns {
		...
		case user.FieldElevated:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field elevated", values[i])
			} else if value.Valid {
				u.Elevated = value.Bool
			}
		}
	}
	return nil
...
func (u *User) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("elevated=")
	builder.WriteString(fmt.Sprintf("%v", u.Elevated))
	builder.WriteByte(')')
