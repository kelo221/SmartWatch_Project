...
// QueryUsername chains the current query on the "username" edge.
func (eq *EventQuery) QueryUsername() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, event.UsernameTable, event.UsernameColumn),
		)
...
func (eq *EventQuery) Clone() *EventQuery {
	...
	return &EventQuery{
		...
		withUsername: eq.withUsername.Clone(),
...
// WithUsername tells the query-builder to eager-load the nodes that are connected to
// the "username" edge. The optional arguments are used to configure the query builder of the edge.
func (eq *EventQuery) WithUsername(opts ...func(*UserQuery)) *EventQuery {
	query := &UserQuery{config: eq.config}
	for _, opt := range opts {
		opt(query)
	}
	eq.withUsername = query
	return eq
}
...
func (eq *EventQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Event, error) {
	...
	if query := eq.withUsername; query != nil {
		if err := eq.loadUsername(ctx, query, nodes, nil,
			func(n *Event, e *User) { n.Edges.Username = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *EventQuery) loadUsername(ctx context.Context, query *UserQuery, nodes []*Event, init func(*Event), assign func(*Event, *User)) error {
